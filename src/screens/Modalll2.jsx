...
import { keizaigaku } from '../../keizaigaku.json';

export default function Modalll2(props) {
  const {
    navigation,
    route,
  } = props;
  ...
  useEffect(() => {
    ...
    const ar = [];
    for (let v = 0; v < keizaigaku.length; v++) {
      if (index === keizaigaku[v].subjectId[0] || index === keizaigaku[v].subjectId[1]) {
        ar.push({
          subject: keizaigaku[v].subject,
          teacher: keizaigaku[v].teacher,
          term: keizaigaku[v].term,
        });
      }
    }
    setSubje(ar);
