...
import { shougaku } from '../../shougaku.json';

export default function Modalll3(props) {
  const {
    navigation,
    route,
  } = props;
  ...
  useEffect(() => {
    ...
    const ar = [];
    for (let v = 0; v < shougaku.length; v++) {
      if (index === shougaku[v].subjectId[0] || index === shougaku[v].subjectId[1]) {
        ar.push({
          subject: shougaku[v].subject,
          teacher: shougaku[v].teacher,
          term: shougaku[v].term,
        });
      }
    }
    setSubje(ar);
