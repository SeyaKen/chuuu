...
import { hougaku } from '../../hougaku.json';

export default function Modalll1(props) {
  const {
    navigation,
    route,
  } = props;
  ...
  useEffect(() => {
    ...
    const ar = [];
    for (let v = 0; v < hougaku.length; v++) {
      if (index === hougaku[v].subjectId[0] || index === hougaku[v].subjectId[1]) {
        ar.push({
          subject: hougaku[v].subject,
          teacher: hougaku[v].teacher,
          term: hougaku[v].term,
        });
      }
    }
