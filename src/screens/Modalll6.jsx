...
import { bunngaku } from '../../bunngaku.json';

export default function Modalll6(props) {
  const {
    navigation,
    route,
  } = props;
  ...
  useEffect(() => {
    ...
    const ar = [];
    for (let v = 0; v < bunngaku.length; v++) {
      if (index === bunngaku[v].subjectId[0] || index === bunngaku[v].subjectId[1]) {
        ar.push({
          subject: bunngaku[v].subject,
          teacher: bunngaku[v].teacher,
          term: bunngaku[v].term,
        });
      }
    }
    setSubje(ar);
