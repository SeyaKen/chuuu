...
import { kokusaikeiei } from '../../kokusaikeiei.json';

export default function Modalll5(props) {
  const {
    navigation,
    route,
  } = props;
  ...
  useEffect(() => {
    ...
    const ar = [];
    for (let v = 0; v < kokusaikeiei.length; v++) {
      if (index === kokusaikeiei[v].subjectId[0] || index === kokusaikeiei[v].subjectId[1]) {
        ar.push({
          subject: kokusaikeiei[v].subject,
          teacher: kokusaikeiei[v].teacher,
          term: kokusaikeiei[v].term,
        });
      }
    }
    setSubje(ar);
