...
import { sougouseisaku } from '../../sougouseisaku.json';

export default function Modalll4(props) {
  const {
    navigation,
    route,
  } = props;
  ...
  useEffect(() => {
    ...
    const ar = [];
    for (let v = 0; v < sougouseisaku.length; v++) {
      if (index === sougouseisaku[v].subjectId[0] || index === sougouseisaku[v].subjectId[1]) {
        ar.push({
          subject: sougouseisaku[v].subject,
          teacher: sougouseisaku[v].teacher,
          term: sougouseisaku[v].term,
        });
      }
    }
    setSubje(ar);
